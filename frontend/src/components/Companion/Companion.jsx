...
const CompanionAnimations = lazy(() => import('./CompanionAnimations'));
...
export default function Companion() {
  ...
  // Preload heavy ~960kB 3D engine in the background right when the dashboard layout is ready and idle,
  // rather than delaying 2 seconds before starting the network fetch.
  useEffect(() => {
    if (isExpanded) {
    ...
    if ('requestIdleCallback' in window && typeof window.requestIdleCallback === 'function') {
      const handle = window.requestIdleCallback(() => setShouldLoad3D(true), { timeout: 3000 });
      return () => window.cancelIdleCallback(handle);
    } else {
      const timer = setTimeout(() => setShouldLoad3D(true), 500);
      ...
      >
        ...
        <div className="panda-chat-avatar-pane">
          <div style={{ width: '100%', height: '85%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {shouldLoad3D ? (
              <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', fontSize: '2rem' }}>🤖</div>}>
  ...
  return (
    ...
    >
      ...
      >
        {shouldLoad3D ? (
          <Suspense fallback={<div style={{ width: 100, height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' }}>🤖</div>}>
            <CompanionAnimations state={state} />
          </Suspense>
        ) : (
          <div style={{ width: 100, height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.5rem', borderRadius: '50%', background: 'rgba(37, 99, 235, 0.08)' }}>🤖</div>
        )}
