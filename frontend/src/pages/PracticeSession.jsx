...
// ── Module Metadata ───────────────────────────────────────────────────────────
const MODULE_META = {
  'sp-1': { label: 'Speaking & Listening Foundations', icon: '🔊', color: '#6366f1', gradient: 'linear-gradient(135deg,#4f46e5,#7c3aed)', category: 'speaking' },
  'sp-2': { label: 'BPO Call Speaking',                icon: '📞', color: '#8b5cf6', gradient: 'linear-gradient(135deg,#7c3aed,#a855f7)', category: 'speaking' },
  'sp-3': { label: 'Fluency & Confidence',             icon: '🎙️', color: '#ec4899', gradient: 'linear-gradient(135deg,#db2777,#f43f5e)', category: 'speaking' },
  'sp-4': { label: 'Advanced Communication',           icon: '⭐', color: '#f59e0b', gradient: 'linear-gradient(135deg,#d97706,#f59e0b)', category: 'speaking' },
...
const SUBMODULE_META = {
  'sp-1-1':'Greetings',            'sp-1-2':'Spelling on Calls',
  'sp-1-3':'Numbers & Time',      'sp-1-4':'Vocabulary & Confidence',
  'sp-2-1':'Formal English',      'sp-2-2':'Workplace Talk',
  'sp-2-3':'Telephone Basics',    'sp-2-4':'Customer Conversations',
  'sp-3-1':'Call Structure',      'sp-3-2':'Active Listening',
  'sp-3-3':'Empathy',             'sp-3-4':'Accent Awareness',
  'sp-4-1':'De-escalation',       'sp-4-2':'Escalation Procedures',
  'sp-4-3':'Global Accents',      'sp-4-4':'Certification Prep',
  'wr-1-1':'Workplace Writing',    'wr-1-2':'SVO & Sentence Formation',
  'wr-1-3':'Tenses & Vocabulary',  'wr-1-4':'Punctuation',
  'wr-2-1':'Professional Tone',    'wr-2-2':'Polite Expressions',
  'wr-2-3':'Email Structure',       'wr-2-4':'Chat Etiquette',
  'wr-3-1':'Business Email',       'wr-3-2':'Empathy & Ownership',
  'wr-3-3':'Complaint & Refund',    'wr-3-4':'Live Chat & CRM Notes',
  'wr-4-1':'Escalation & Conflict','wr-4-2':'SOPs & Compliance',
  'wr-4-3':'Timed Writing',        'wr-4-4':'Production Simulations',
};
...
function SubModulePicker({ moduleId, doneSubModules, onSelect, onBack, onNavigate }) {
  const m = MODULE_META[moduleId];
  const subIds = [`${moduleId}-1`, `${moduleId}-2`, `${moduleId}-3`, `${moduleId}-4`];
  const subIcons = ['📖', '✏️', '🎯', '🏅'];
  const allSubsDone = subIds.every(id => doneSubModules.includes(id));
    ...
    <div>
      ...
      <div style={{ ...S.heroCard, background:m.gradient }}>
        ...
        <div style={{ fontSize:13, color:'rgba(255,255,255,0.8)' }}>
          {doneSubModules.filter(s => subIds.includes(s)).length} / {subIds.length} practice drills completed
        </div>
      </div>
      ...
      <div style={S.grid}>
        {subIds.map((subId, idx) => {
          ...
          // Unlock if it's the first one, or if the previous one is done
          const unlocked = idx === 0 || doneSubModules.includes(subIds[idx - 1]);
          const data = FALLBACK[subId];
          return (
            <div key={subId}
              onClick={() => unlocked && onSelect(subId)}
              style={{ background:'#fff', borderRadius:16, padding:'20px', border:`2px solid ${done?'#86efac':unlocked?m.color+'30':'#e2e8f0'}`, cursor:unlocked?'pointer':'not-allowed', opacity:unlocked?1:0.5, transition:'all 0.15s' }}
              onMouseEnter={e => unlocked && (e.currentTarget.style.transform='translateY(-2px)')}
              onMouseLeave={e => (e.currentTarget.style.transform='translateY(0)')}
            >
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
                <div style={{ width:44, height:44, borderRadius:12, background:done?'#d1fae5':unlocked?m.color+'18':'#e2e8f0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>
                  {done ? '✅' : !unlocked ? '🔒' : subIcons[idx]}
                </div>
                {unlocked && (
                  <span style={S.badge(done?'#10b981':m.color)}>{done ? 'Completed' : `${data?.questions?.length || 5} Questions`}</span>
                )}
              </div>
              <div style={{ fontSize:11, fontWeight:700, color:unlocked?'#94a3b8':'#cbd5e1', textTransform:'uppercase', letterSpacing:1, marginBottom:4 }}>Sub-module {idx + 1}</div>
              <div style={{ fontSize:15, fontWeight:700, color:unlocked?'#1e293b':'#94a3b8', marginBottom:4 }}>{SUBMODULE_META[subId]}</div>
              <div style={{ fontSize:12, color:'#94a3b8' }}>
                {!unlocked ? 'Complete previous drill to unlock' : m.category === 'speaking' ? '🎤 Speaking drills — read aloud & record' : '✍️ Multiple choice writing exercises'}
              </div>
            </div>
          );
        })}
        ...
        {/* Assessment Card */}
        <div
          onClick={() => allSubsDone && onNavigate('assessment')}
          style={{ background:allSubsDone?'linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)':'#fff', borderRadius:16, padding:'20px', border:`2px solid ${allSubsDone?'#f59e0b':'#e2e8f0'}`, cursor:allSubsDone?'pointer':'not-allowed', opacity:allSubsDone?1:0.5, transition:'all 0.15s' }}
          onMouseEnter={e => allSubsDone && (e.currentTarget.style.transform='translateY(-2px)')}
          onMouseLeave={e => (e.currentTarget.style.transform='translateY(0)')}
        >
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
            <div style={{ width:44, height:44, borderRadius:12, background:allSubsDone?'#fef08a':'#e2e8f0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>
              {allSubsDone ? '🏆' : '🔒'}
            </div>
            {allSubsDone && (
              <span style={S.badge('#d97706')}>Required</span>
            )}
          </div>
          <div style={{ fontSize:11, fontWeight:700, color:allSubsDone?'#b45309':'#cbd5e1', textTransform:'uppercase', letterSpacing:1, marginBottom:4 }}>Final Step</div>
          <div style={{ fontSize:15, fontWeight:800, color:allSubsDone?'#92400e':'#94a3b8', marginBottom:4 }}>Module Assessment</div>
          <div style={{ fontSize:12, color:allSubsDone?'#b45309':'#94a3b8' }}>
            {allSubsDone ? 'Click to take the assessment and earn your certificate!' : 'Complete all 4 drills to unlock the assessment.'}
          </div>
        </div>
      </div>
    </div>
...
function SpeakingPractice({ subModuleId, moduleId, onComplete, onBack, data }) {
  const m      = MODULE_META[moduleId];
...
function WritingPractice({ subModuleId, moduleId, onComplete, onBack, data }) {
  const m     = MODULE_META[moduleId];
...
export default function PracticeSession({ onNavigate }) {
  ...
  // Hide the 3D companion character during active practice sessions
  useEffect(() => {
    const companion = document.querySelector('.panda-companion-container');
    if (!companion) return;
    if (screen === 'speaking' || screen === 'writing') {
      companion.style.display = 'none';
    } else {
      companion.style.display = '';
    }
    return () => { if (companion) companion.style.display = ''; };
  }, [screen]);
  ...
  useEffect(() => {
    ...
    // Fetch user's actual lesson attempts instead of non-existent practice endpoints
    api.get('/lesson/attempts')
      .then(res => {
        const attempts = res.data || [];
        setAllAttempts(attempts);
        
        // Extract main module IDs (e.g. 'sp-1' from 'sp-1-1')
        const attemptedModuleIds = new Set();
        attempts.forEach(att => {
          if (att.lessonId) {
            // Usually format is 'sp-1-1', so substring(0, 4) gives 'sp-1'
            const mainModuleId = att.lessonId.split('-').slice(0, 2).join('-');
            attemptedModuleIds.add(mainModuleId);
          }
        });
        setCompletedModules(Array.from(attemptedModuleIds));
      })
  ...
  const handleSelectModule = (moduleId) => {
    ...
    // Sub-modules that belong to this module and have been attempted
    const completed = allAttempts
      .filter(att => att.lessonId && att.lessonId.startsWith(moduleId))
      .map(att => att.lessonId);
    setDoneSubModules([...new Set(completed)]);
  };

  const handleSelectSubModule = async (subModuleId) => {
    setSelectedSubModule(subModuleId);
    const category = MODULE_META[selectedModule]?.category;
    
    setPracticeLoading(true);
    try {
      // The new backend practice module handles fetching unique sets based on prior attempts
      const res = await api.get(`/practice/questions/${subModuleId}`);
      let formattedData = { questions: [] };
      
      if (res.data) {
        if (category === 'speaking') {
          // For speaking, use partB for speaking prompts or fallback to partA text if partB is empty
          const rawQuestions = res.data.partB?.length > 0 ? res.data.partB : res.data.partA;
          formattedData.questions = (rawQuestions || []).map(q => ({
            text: q.question,
            tip: q.explanation || q.answer || 'Focus on clarity and pronunciation.'
          }));
        } else {
          // For writing, use partA (MCQs)
          formattedData.questions = (res.data.partA || []).map(q => {
            const correctIdx = q.correctOption === 'A' ? 0 : q.correctOption === 'B' ? 1 : q.correctOption === 'C' ? 2 : 3;
            return {
              question: q.question,
              options: [q.optionA, q.optionB, q.optionC, q.optionD].filter(Boolean),
              correct: correctIdx,
              explanation: q.explanation || q.answer || 'Review the correct grammar structure.'
            };
          });
        }
      }
      
      // Fallback if backend returned no questions
      if (!formattedData.questions || formattedData.questions.length === 0) {
        formattedData = FALLBACK[subModuleId];
      }
      
      setPracticeData(formattedData);
    } catch (err) {
      console.warn('Failed to fetch practice questions, using fallback', err);
      setPracticeData(FALLBACK[subModuleId]);
    } finally {
  ...
  return (
    <div style={S.page}>
      ...
      {modulesLoading ? (
        ...
        <>
          {practiceLoading ? (
            <Spinner text="Generating personalized practice set..." />
          ) : (
              <SubModulePicker
