...
export class GamesService {
  ...
  async getAllPersonalBests(userId: string) {
    const gameTypes = [
      ...
      // Grammar
      'GRAMMAR_NINJA', 'SENTENCE_SURGEON', 'ERROR_HUNT', 'TENSE_TRANSFORMER',
      // BPO
    ...
    const bests = await Promise.all(
      gameTypes.map(async gameType => {
        const best = await this.sessionRepo.findOne({
          where: { userId, gameType, completed: true },
          order: { score: 'DESC' },
          select: ['gameType', 'category', 'score', 'accuracy', 'completedAt'],
        });
        return best || { gameType, score: 0, accuracy: 0, completedAt: null };
      }),
    );
    ...
    for (const best of bests) {
      const cat = (best as any).category || 'UNCATEGORIZED';
      if (!grouped[cat]) grouped[cat] = [];
      grouped[cat].push(best);
    }
