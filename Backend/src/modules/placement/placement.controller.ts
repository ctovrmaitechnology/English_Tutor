...
export class PlacementController {
  ...
  async start(@Request() req) {
    return this.placementService.startAttempt(req.user.id, req.user.username);
  }
  ...
  async finish(@Request() req, @Body() body: FinishAttemptDto) {
    return this.placementService.finishAttempt(req.user.id, body.attemptId, body.answers);
  }
